...
#if defined(NDEBUG)
# error "Crixcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x079ae07091ef64250c49f22b2ec02cd3ab2ac3e9112f96a72ac94aeb4b1fa86b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Crixcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Crixcoin Signed Message:\n";
...
{
    // Crixcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Crixcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Crixcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Crixcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Crixcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Crixcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1cc75763c109c5771741268a8dfa4417ba8a641d3d8efd37f6210ece051edf47");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "FAZ 07/Jun/2019 Amerikas Tech-Aktien sind ueber dem Zenit";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042e8890fb7da054268e8f4d8efb3eaace9fab533168924cc3f792e7030c769e934349d63fe29b38b5b0fc591469f9fc32720a28e3ea027a5d5a4e459074940295") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1563976846;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084983500;

        if (fTestNet)
        {
            block.nTime    = 1563976815;
            block.nNonce   = 387445328;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xaa2b99f55f26db6bee4f8dde8f17d543be25d11e6438114de581c028b590f672"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc7, 0xb3, 0xdc }; // Crixcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CrixcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CrixcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CrixcoinMiner(CWallet *pwallet)
{
    printf("CrixcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("crixcoin-miner");
    ...
    try { loop {
        ...
        printf("Running CrixcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CrixcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CrixcoinMiner, pwallet));
}
